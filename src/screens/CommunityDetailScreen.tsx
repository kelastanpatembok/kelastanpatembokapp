...
export default function CommunityDetailScreen() {
  ...
  const loadData = async () => {
    try {
      // Load platform
      const platformSnap = await firebaseDb
      ...
      // Load community
      const communityDoc = await firebaseDb
        .collection('platforms')
        .doc(platformData.id)
      ...
      if (uid) {
        ...
        if (isPlatformOwner) {
          ...
          const memberDoc = await firebaseDb
            .collection('platforms')
            .doc(platformData.id)
            .collection('members')
            .doc(uid)
  ...
  const handleAddPost = async (content: string, imageUri?: string) => {
    if (!firebaseAuth.currentUser || !platform || !community) return;
